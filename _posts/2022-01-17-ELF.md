...
where $\chi_1 = \psi_1 \otimes \beta_1$. $\psi$ is the spacial orbital and $\beta$ (or for other spin channel $\alpha$) is the spin orbital ($\int \alpha(w) \beta(w) dw = 0$; $\int \beta(w) \beta(w) dw = 1$).
