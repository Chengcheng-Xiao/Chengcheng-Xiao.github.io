...
using the properties of $\rho(r_{1},r_{2})$, we can see that,
...
i.e., if a $\sigma$ spin electron is definitely at point $r_1$, then the total probability of finding another $\sigma$-spin electron elsewhere in the system is $N - 1$. In our case, $N=2$ so the total probability of finding another $\sigma$-spin electron is 1.

<!-- Now, we need to figure out how to make calculating this easier. -->

Now, if we take the the second term from Eq. 7, we see that it has the unit of a charge density. This, turns out to be the so-called the exchange charge density (See [becke1983](https://onlinelibrary.wiley.com/doi/abs/10.1002/qua.560230605)):
...
we see that we can express the Taylor expansion at a specific point ($a$) as:
...
Using Eq. 11, we can then Taylor expand the exchange charge density $\rho_{x}^{\sigma \sigma} (r_1,r_2)$ as $\vec r_2 \to \vec r_1$:
...
Here, $\nabla_{r_2}$ is the derivative along $\vec s$ direction and is angle independent.
...
Okay, now we have the form of the spherically averaged Taylor expansion operator in Eq. 14, plugging this back into Eq. 12 gives us:
...
Note that we are Taylor expanding at $r_2 \to r_1$, thus, we have changed all $r_2$ dependence to $r_1$ (and then omitted all $r$ dependence since all $r$ depend on $r_1$) after acting the $\nabla$ operator .
...
Remember that what we really want is the Taylor expansion of $P^{\sigma \sigma}_\text{cond} (r_1, r_2)$.  So, instead of $\left\langle \rho_x (\vec r_1, \vec r_2) \right\rangle$ in Eq. 15, we now write:
...
Because this is the Bosonic version of the positively defined kinetic energy density (some times also called the von_Weizacker kinetic energy density), it's the lower bound of the Fermion version:
