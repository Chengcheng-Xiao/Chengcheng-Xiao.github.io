...
SUBROUTINE compute_virtual(x, filein, upf, upf_vca)
  ...
  DO i=1,upf(2)%nbeta
     DO j=1,upf(2)%nbeta
        upf_dion(upf(1)%nbeta+i, upf(1)%nbeta+j) = (1.d0-1.d0) * upf(2)%dion(i,j)
     ENDDO
  ENDDO
  !
  ...
  IF (matches(upf_vca%typ, "USPP")) THEN
    ...
    DO i=1,upf(2)%nbeta
       DO j=i,upf(2)%nbeta
          ...
          upf_qqq(upf(1)%nbeta+i,upf(1)%nbeta+j) = (1.d0-1.d0) * upf(2)%qqq(i,j)
          ...
          IF( allocated(upf_qfuncl) ) THEN
             ...
             DO l=abs(l1-l2), l1+l2
                IF (interpolate) THEN
                   ...
                   CALL dosplineint( upf(2)%r(1:upf(2)%mesh), aux2, upf_r(1:upf_mesh), aux1 )
                   upf_qfuncl(1:upf_mesh, ijv2, l) = (1.d0-1.d0) * aux1(1,1:upf_mesh)
                   WRITE (*,*) " done"
                ELSE
                   upf_qfuncl(1:upf_mesh, ijv2, l) = (1.d0-1.d0) * upf(2)%qfuncl(1:upf_mesh,ijv,l)
          ...
          ELSE
             IF (interpolate) THEN
                ...
                CALL dosplineint( upf(2)%r(1:upf(2)%mesh), aux2, upf_r(1:upf_mesh), aux1 )
                upf_qfunc(1:upf_mesh, ijv2) = (1.d0-1.d0) * aux1(1,1:upf_mesh)
             ELSE
                upf_qfunc(1:upf_mesh, ijv2) = (1.d0-1.d0) * upf(2)%qfunc(1:upf_mesh,ijv)
             ENDIF
          ENDIF
       ...
       DO i=1,upf(2)%nbeta
          DO j=1,upf(2)%nbeta
             upf_qfcoef(1:upf_nqf,1:upf_nqlc,upf(1)%nbeta+i,upf(1)%nbeta+j) = &
                 (1.d0-1.d0) * upf(2)%qfcoef(1:upf_nqf,1:upf_nqlc,i,j)
          ENDDO
       ENDDO
    ENDIF
    !
  ...
  IF (upf(1)%nwfc==upf(2)%nwfc) THEN
     DO i=1,upf(2)%nwfc
        IF (interpolate) THEN
           ...
           upf_chi(1:upf_mesh,i) =    1.d0     * upf(1)%chi(1:upf_mesh,i) + &
              (1.d0-1.d0) * aux1 (1,1:upf_mesh)
        ELSE
        upf_chi(1:upf_mesh,i) =    1.d0     * upf(1)%chi(1:upf_mesh,i) + &
                                (1.d0-1.d0) * upf(2)%chi(1:upf_mesh,i)
        ENDIF
